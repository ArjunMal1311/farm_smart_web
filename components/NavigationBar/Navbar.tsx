...
const Navbar = ({ currentUser }: UserProps) => {
    ...
    return (
        <div className={`flex justify-center w-full border-b-2 bg-lime-100 select-none ${InterScript.className}`}>
            <div className='flex items-center justify-between w-[1200px] gap-4 md:gap-0 mx-6 my-2 pt-2'>
                <div className='flex items-center space-x-2 cursor-pointer' onClick={() => { router.push("/") }}>
                    <Image src={"/logo.jpg"} height={40} width={40} alt="" className='rounded-lg'/>
                    <div className={`sm:text-[20px] text-[10px] font-extrabold uppercase leading-[24.2px] tracking-widest ${JetBrains.className}`}>Farm Smart</div>
                </div>
                <div className='relative' ref={dropdownRef}>
                    <div className='flex flex-row items-center gap-3'>

                        <Link href='/calculator' className='text-bold border shadow-lg  hover:bg-gray-200 border-black  px-3 py-1 rounded-lg cursor-pointer' >
                            Calculators
                        </Link>

                        <Link href='/form' className='text-bold border shadow-lg  hover:bg-gray-200 border-black  px-3 py-1 rounded-lg cursor-pointer' >
                            Form
                        </Link>

                        <div onClick={toggleOpen} className='p-4 md:py-2 md:px-3 border shadow-lg border-black flex flex-row items-center gap-3 rounded-full cursor-pointer hover:shadow-md transition' >
                            <HiOutlineMenuAlt1 />
